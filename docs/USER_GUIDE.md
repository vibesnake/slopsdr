...
short-term movement; positions 1 through 100 map exponentially from an 8 ms to
a 1.8 second averaging time constant, so low settings remain responsive while
the maximum is deliberately strong. The selected value persists.

AVG uses frame timestamps and averages normalized values directly on the
displayed dBFS axis, so its rise and fall look symmetric and remain consistent
when FFT cadence changes. Enabling it starts from the newest frame without
fading up from zero. Hardware retunes,
...
delay. It uses the same compact, scrollable rows as the bookmark list, without
bookmark scanner-inclusion checkboxes. Enter a unique name and select
**Save New**. Single-click a row to select it, double-click to **Load** a
disconnected copy into the current fields, or use **Update** (and optionally
rename it) or **Delete** after confirmation. Loading never starts the scanner.
Drag rows by their handle to reorder them; the order and stable preset
identities persist across restarts. Presets retain invalid-for-now bounds
visibly, so correct them before starting a scan.
...
When zoomed, use the horizontal scrollbar below the waterfall to pan both
displays across the captured span without changing receiver tuning or scanner
state. The scrollbar remains unavailable at full-span zoom.
...
recording controls in its top row. Seekable sources use the full-width lower-row
seek bar: drag to preview and release to seek by decoded WAV frame or raw-IQ
complex sample. **Stop**
...
there is no loop, playlist, compressed-codec playback, or scanner
